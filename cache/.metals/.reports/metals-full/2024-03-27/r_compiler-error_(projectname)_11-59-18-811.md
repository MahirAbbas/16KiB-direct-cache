...
### java.lang.StringIndexOutOfBoundsException: offset 2093, count -7, length 2118
...
case class L1CacheIO(addressWidth : Int) extends Bundle with IMasterSlave {
    ...
    val dataLoadReq = Stream(DataLoadCmd(addressWidth)) 
    val dataLoadRsp = Flow(DataLoadRsp(addressWidth)) 
    // add signal for stall in case cache doesnt have data?
    // 
    val dataStoreReq = Stream(DataStoreCmd(addressWidth))
    val dataStoreRsp = Flow(DataStoreCmd(addressWidth))
